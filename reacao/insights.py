...
import os
from .types import Event, Insight, Segment
...
def _template(ev: Event, trecho: str) -> Insight:
    texto = (f"Aos {_mmss(ev.t_ini)}, no momento '{ev.momento}', enquanto o pregador dizia \"{trecho[:160]}\", "
             f"a reação observada foi {NOMES.get(ev.tipo, ev.tipo)}: {ev.sinal} variou {ev.magnitude_pp:+.0f} p.p. "
...
def write(events: list[Event], segments: list[Segment], max_insights: int = 8) -> list[Insight]:
    ...
    for ev in events:
        trecho = text_between(segments, ev.t_ini - 30, ev.t_fim) or "(sem transcrição nesta janela)"
